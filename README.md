...
6. **時間領域解析**：`scripts/analyze_time_domain.m` で IAE/ISE/ITAE を再計算し、`metrics.json` に整形（Simulink 内数値と ±1e-9 で照合）。
7. **レポート生成**：`scripts/export_report.m` で波形 PNG・誤差 PNG・監視 CSV・`results.md` を `artifacts/sim/<run_id>/report/` に生成（再実行でバイト一致）。
8. **可視化**：出力応答（追従誤差、立上り時間、整定時間、オーバーシュート）を数値比較。
9. **ユーザ別制御ブロック**：ユーザセグメント毎に LSTM ブロックを切替（例：ルックアップテーブル + スイッチ）。
