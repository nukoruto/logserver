...
k_const = create_block(model_name, 'k_const', 'simulink/Sources/Constant', ...
    [910 60 960 90], struct('Value', num2str(cfg.check.k_of_n.k, '%.12g')));
n_const = create_block(model_name, 'n_const', 'simulink/Sources/Constant', ...
    [970 60 1020 90], struct('Value', num2str(cfg.check.k_of_n.n, '%.12g')));
hyst_up_const = create_block(model_name, 'hyst_up_const', 'simulink/Sources/Constant', ...
    [1030 60 1080 90], struct('Value', num2str(cfg.check.k_of_n.hyst_up, '%.12g')));
hyst_down_const = create_block(model_name, 'hyst_down_const', 'simulink/Sources/Constant', ...
    [1090 60 1140 90], struct('Value', num2str(cfg.check.k_of_n.hyst_down, '%.12g')));
check_sink = add_to_workspace(model_name, 'check_sink', 'check_flags', [1080 170 1190 220]);
k_count_sink = add_to_workspace(model_name, 'k_count_sink', 'k_of_n_count', [1080 230 1190 280]);
...
add_line(model_name, 'k_const/1', 'k_of_n_latch/2', 'autorouting', 'on');
add_line(model_name, 'n_const/1', 'k_of_n_latch/3', 'autorouting', 'on');
add_line(model_name, 'hyst_up_const/1', 'k_of_n_latch/4', 'autorouting', 'on');
add_line(model_name, 'hyst_down_const/1', 'k_of_n_latch/5', 'autorouting', 'on');
add_line(model_name, 'k_of_n_latch/1', 'check_sink/1', 'autorouting', 'on');
add_line(model_name, 'k_of_n_latch/2', 'k_count_sink/1', 'autorouting', 'on');
...
function script = build_k_of_n_script(~)
script = [ ...
    'function [alarm,countK] = k_of_n_latch_wrapper(viol,k_in,n_in,hyst_up_in,hyst_down_in)\n' ...
    '%%#codegen\n' ...
    '[alarm,countK] = k_of_n_latch(viol, k_in, n_in, hyst_up_in, hyst_down_in);\n' ...
    'end'];
end
