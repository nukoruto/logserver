...
k_block = create_block(model_name, 'k_of_n_latch', 'simulink/User-Defined Functions/MATLAB Function', ...
    [910 150 1060 260], struct());
set_param(k_block, 'MATLABFcn', build_k_of_n_wrapper());

k_const = create_block(model_name, 'k_of_n_const_k', 'simulink/Sources/Constant', ...
    [910 270 960 320], struct('Value', num2str(cfg.check.k_of_n.k, '%.12g'), 'OutDataTypeStr', 'double'));
n_const = create_block(model_name, 'k_of_n_const_n', 'simulink/Sources/Constant', ...
    [970 270 1020 320], struct('Value', num2str(cfg.check.k_of_n.n, '%.12g'), 'OutDataTypeStr', 'double'));
hyst_up_const = create_block(model_name, 'k_of_n_const_hup', 'simulink/Sources/Constant', ...
    [1030 270 1080 320], struct('Value', num2str(cfg.check.k_of_n.hyst_up, '%.12g'), 'OutDataTypeStr', 'double'));
hyst_down_const = create_block(model_name, 'k_of_n_const_hdown', 'simulink/Sources/Constant', ...
    [1090 270 1140 320], struct('Value', num2str(cfg.check.k_of_n.hyst_down, '%.12g'), 'OutDataTypeStr', 'double'));

check_mux = create_block(model_name, 'check_mux', 'simulink/Signal Routing/Mux', ...
    [1080 150 1130 240], struct('Inputs', '2'));
check_spec = create_block(model_name, 'check_spec', 'simulink/Signal Attributes/Signal Specification', ...
    [1150 170 1240 220], struct('OutDataTypeStr', 'double', 'Dimensions', '2', 'SignalName', 'check_flags'));
check_sink = add_to_workspace(model_name, 'check_sink', 'check_flags', [1260 170 1370 220]);
...
add_line(model_name, 'k_of_n_const_k/1', 'k_of_n_latch/2', 'autorouting', 'on');
add_line(model_name, 'k_of_n_const_n/1', 'k_of_n_latch/3', 'autorouting', 'on');
add_line(model_name, 'k_of_n_const_hup/1', 'k_of_n_latch/4', 'autorouting', 'on');
add_line(model_name, 'k_of_n_const_hdown/1', 'k_of_n_latch/5', 'autorouting', 'on');
add_line(model_name, 'k_of_n_latch/1', 'check_mux/1', 'autorouting', 'on');
add_line(model_name, 'k_of_n_latch/2', 'check_mux/2', 'autorouting', 'on');
add_line(model_name, 'check_mux/1', 'check_spec/1', 'autorouting', 'on');
add_line(model_name, 'check_spec/1', 'check_sink/1', 'autorouting', 'on');
...
function script = build_k_of_n_wrapper()
script = [ ...
    'function [alarm,countK] = k_of_n_latch_wrapper(viol, K, N, hyst_up, hyst_down)\n' ...
    '%%#codegen\n' ...
    '[alarm,countK] = util.k_of_n_latch(viol, K, N, hyst_up, hyst_down);\n' ...
    'end'];
end
